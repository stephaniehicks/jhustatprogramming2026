...
#### Arguments
# e.g. if you want to pass na.rm=TRUE to mean(),
...
#     the cars with 4, 6, and 8 cylinders respectively.
by_cyl <- split(mtcars, mtcars$cyl)
length(by_cyl)
str(by_cyl)

# fit a linear model to understand how the miles per
#     gallon (`mpg`) associated with the weight (`wt`)
...
# fit the model with purrr (returns a list from each
#     lm fit for each cylinder)
by_cyl |>
...
#### walk() and friends
# when you want to apply a function (e.g. cat(), write_csv(), or ggsave()), 
#      but do not return output, try walk and friend
